...
import de.tubs.isf.guido.verification.systems.cpachecker.CmdLineArgument.*;
          ...
          if (configFile != null) {
            try {
              ...
              try {
            	...
            	System.out.printf("Invalid configuration %s (%s)", argName, e.getMessage());
            }
          } else {
        	  System.out.printf(
                "Invalid option %s\n"
        	...
        	System.out.printf("Invalid option %s", arg);
        }
    ...
    for (Map.Entry<String, String> template : DEFAULT_CONFIG_FILES_TEMPLATES.entrySet()) {
      ...
      if (file != null) {
        if (!template.getValue().isEmpty()) {
        	System.out.printf(template.getValue(), pName);
        }
        return file;
      }
    }
